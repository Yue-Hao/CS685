...
classdef RRT < Navigation
    ...
    methods
        ...
        % Plot the velocity and steering angle profiles required to move from start to goal.
        function p = path_control(rrt, xstart, xgoal)
        ...
        % Add a local planner which will move from the initial pose to the closest vertex.
        function p = local_planner(rrt, xstart, xclosest)

            g = rrt.graph;
            vclose = g.closest(xstart);
            xclose = g.coord(vclose);
            ...
            while(1)
                ...
                best = bestpath(local_path(:,end), xclose, ntrials);
                local_path = [local_path best.path(:,best.k)];
                best.d
                if(best.d < 0.1)
                    break;
                end
...
for i=1:N
    ...
    % Gaussian distribution steering angle
    steer = randn * steermax;
